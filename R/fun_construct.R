...
#' from a vector of knots and the matrix of suggested derivatives.
#' The matrix is tested for its correctness as in \code{is.splinets} and adjusted using one of the implemented methods.
#' @param knots \code{n+2} vector, the knots over which a spline is built; 
#' There should be at least \code{2*smorder+4} of them. 
#' @param smorder integer, the order of smootheness;
#' @param matder \code{(n+2)x(smorder+1)} matrix, the matrix of derivatives (will be corrected if does not correspond to a spline).
...
#' }
#' Default is \code{'RRM'}, see the R-journal paper, for further details.
#' @return A \code{Splinets} object of the length one corresponding to a single spline.
#' @details The function constructs a \code{Splinet} object only over a single interval support. 
#' Combining this function and 
#' function \code{lincom} allows to introduce a multi-component support.  
#' @export
#' 
...
#' \code{\link{lincomb}} for creation of more complex than a single interval support sets; 
