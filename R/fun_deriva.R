...
#' @description The function generates a \code{Splinets}-object which contains all the 
#' derivative functions of the splines from the input \code{Splinets}-object.
#' The function also verifies the support set of the output to provide the accurate information about 
#' the support sets by excluding regions over which the original function was constant. 
...
#' to be zero and the corresponding knots will be removed from the support. Default value is 1e-7.  
#' @return An \code{Splinets} object of the order \code{k-1} that contains also the updated information about the support set.
#' @export
...
deriva = function(object, epsilon = 1e-7){
  ...
  for(i in 1:d){
    S[[i]] = S[[i]][,-1,drop=F] #removing the first column is equivalent to taking the derivative
  ...
  object@der = S
  object@smorder = k-1
  object@taylor = object@taylor[, -(k+1), drop = F]
  object@supp = supp
