...
#' which is a union of disjoint \code{Nsupp} intervals. Outside the set (support), the spline vanishes. 
...
#' @slot taylor \code{(n+1) x (smorder+1)}, if \code{equid=F}, or \code{1 x (smorder+1)}
#' if \code{equid=T},  columnwise vectors  of the Taylor expansion coefficients at the knots; 
...
#' The columns of this matrix is used for evaluation of the values of the splines in-between knots,
#' see the paper on the package for further details.
...
#'   \item \code{gsob} for Gram-Schmidt osplines,
#'   \item \code{twob} for two-sided osplines,
