...
#' 
#' @description Any spline of a given order remains a spline of the same order if one considers it on a larger set of knots than the original one.
#'              However, this changes \code{Splinets} representation of the so-refined spline. 
...
#' @param newknots \code{m} vector, the new knots; 
#' The knots do not need to be ordered and repetitions from the input \code{Splinets}-object knots are allowed since any ties are merged into single knots. 
#' @return An \code{Splinet} object with the new refined knots and the new order with the matrix of derivative at the knots evaluated, correspondingly. 
#' @details The function utilizes \code{deriva()}-function to evaluate the derivative at the refined knots. 
#' It removes duplications of the refined knots, and account also for the not-fully supported case.
#' In the case if the range of the additional knots extends beyond the knots of the input \code{Splinets}-object,
#' the support sets for the output \code{Splinets}-object account for smaller than the full support. 
#' @export
#' @seealso \code{\link{deriva}} for computing derivatives at selected points; 
#' \code{\link{project}} for an orthogonal projection/decomposition to a set of splines;
#' 
...
if(length(supp) == 0 & prod(range(newxi)==range(xi))){#It means that the full support is assumed for each spline in the object
   ...
   for(r in 1:d){#running through the splines in the object
     for(i in 1:(k+1)){#running through the columns of the matrix of derivatives (one-sided version for the highest derivative)
       ...
       newS[,i]=evsp[,2,drop=F] 
     }
     newS=sym2one(newS,inv=T) #the way the evaluation is performed on the the zero order splines makes it one-sided values at the last column
