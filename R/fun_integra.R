...
#' (unless the definite integral of the input is zero, in which the case the support is the same as the input support). 
#' @param object a \code{Splinets} object of the smoothness order \code{k};
#' @param epsilon non-negative number indicating accuracy when close to zero value are identifying; This is useful in 
#' identification if the boundary conditions of the integral are satisfied. 
#' @return a \code{Splinets} object with order \code{k+1} which contains the indefinite integrals
#' of the input \code{Splinets}.
#' @details  
...
#' @seealso \code{\link{deriva}} for generating derivative functions of splines; \code{\link{dintegra}} for the definite integral;
  ...
  for(i in 1:d){ #running through the splines in the object
    ...
    if(Nsupp>1){#if there is more than one component in the support
      ...
      if(abs(nS[n+2,1])<epsilon){#the case of the spline (the definite integral of the object is zero)
        ...
        aa=exsupp(sym2one(nS,inv=T)) #extracting support from the full support matrix
    ...
    if(FullSupp==TRUE){
      newS[[i]]=sym2one(nS,inv=T) #
      }else{
      newS[[i]]=sym2one(nS,supp[[i]],inv=T) #one needs to get back to the symmetric representation
      }
  }
