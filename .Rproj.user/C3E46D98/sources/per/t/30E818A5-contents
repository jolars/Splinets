...
lincomb_full = function(S, supp, k, n, A, SuppCont){
  ...
  if(SuppCont){
    for(i in 1:l){
      S_new[[i]] = sym2one(matrix(temp_matrix[,i], byrow = F, nrow = n+2, ncol = k+1), inv = T)
...
dint_engine = function(knots, supp, taylor, S){
  d =  dim(supp)[1]
  SS = der_split(S, supp)
  res = dint_single(knots, supp[1,,drop=F],taylor,SS[[1]])
  if(d > 1){
    for(i in 2:d){
      res = res + dint_single(knots, supp[i,,drop=F],taylor,SS[[i]])
    }
  }
  return(res)
}
...
int_single = function( taylor, S, c_0 = 0){
  ...
  S = sym2one(S,inv =T) #S[(n+2),1] typically is non-zero
  return(S)
}
...
int_engine = function(supp, taylor, S ,epsilon = 1e-7){
  ...
  beg=1 #evaluation of the current beginning in S corresponding to the component (should be equal to one)
    ...
    for(i in 1:Nsupp){
      ...
      AA = int_single(loc_taylor, S[beg:(beg+len),,drop=F],c_0=c_0)
...
coeff1 <- function(xi, k){
  matrix(1/c(matrix(rep(diff(head(xi, -1), k), k+1), byrow = T, nrow = k+1)), ncol = 1)
}

coeff2 <- function(xi, k){
  matrix(1/c(matrix(rep(-diff(tail(xi, -1), k), k+1), byrow = T, nrow = k+1)), ncol = 1)
}
...
grscho=function(A, H){
  ...
  B[,1]=A[,1, drop=F]/sqrt(H[1,1]) #the first output vector normalized
  for(i in 1:(nb-1)){
    A[,(i+1):nb]=(A[,(i+1):nb]-(A[,i, drop=F]%*%H[i,(i+1):nb,drop=F])/H[i,i])
    H[(i+1):nb,(i+1):nb]=(H[(i+1):nb,(i+1):nb]-t(H[i,(i+1):nb,drop=F])%*%H[i,(i+1):nb,drop=F]/H[i,i])
    B[,i+1]=A[,i+1,drop=F]/sqrt(H[i+1,i+1]) #subsequent column to the output
  }
  B
}
...
net_structure = function(n_so, k){
  ...
  # tuplet
  temp_res[,,3] = matrix(rep(1:n_tuplets, k), byrow = T, nrow = k)
  # basis
...
sym2one_single = function(S, inv=F){
  n = dim(S)[1]
  l = dim(S)[2]
  d = ceiling(n/2)
  if(inv){
...
supp_union_bin = function(supp1, supp2){
  ...
  supp = matrix(as.numeric(names(temp))[temp==1], byrow = T, ncol = 2)
  return(supp)
}
...
innerdb = function(fdata,basis){
  ...
  for(i in 1:(n-k+1)){#looping through the elements of the basis
    ...
    intgr_val=intgr_val[,-1,drop=F]
    diff_int=diff(intgr_val)
    
    A[,i]=t(y[ind_in_range,,drop=F])%*%diff_int
  }
...
#   
#   A=t(fdata[,-1,drop=F])%*%DA #This is N x n_basis, it represents the inner products of 
