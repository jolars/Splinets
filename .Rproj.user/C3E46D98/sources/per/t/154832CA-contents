...
#' 
#' @description  The projection of splines or functinal data into the linear spline space spanned on a given set of knots.  
#' @param fdsp \code{Splinets}-object or a \code{n x (N+1)} matrix, the representation of \code{N} functions to be projected to the space spanned by \code{Splinets}-basis over a specific set of knots;
#' If the parameter is a \code{Splinets}-object containing \code{N} splines, then it is orthogonally projected or represented in the basis that is specified by other parameters. 
#' If the paramater is a matrix, it is then treated as \code{N} piecewise constant functions with the argument values in the first column and the corresponding values of the functions in the remaining \code{N} columns. 
#' @param knots vector, the knots of the projection space, together with \code{smorder} fully characterizes the projection space; This parameter is overridden by the SLOT \code{basis@knots} of \code{basis} input if this one is not \code{NULL}.
...
#' @param basis \code{Splinets}-object, the basis used for representation of \code{fdsp};
...
#' @param graph logical, indicator if the illustrative plots should be produced;
#' @return The value of the function is a list made of the three elements
#' \itemize{
#'   \item \code{'project$coeff'} N x (n-k+1) matrix of the coefficients of representation of the projection of the input in the splinet basis,
#'   \item \code{'project$basis'} the splinet basis,
#'   \item \code{'projedt$sp'} the \code{Splinets}-object containing the projected splines.
#'  } 
#' @details The obtained coefficients \eqn{\mathbf A = (a_{ji})} with respect basis allows to evaluate the splines \eqn{S_j} corresponding to the projection according to the relation
#' \deqn{
#' S_j=\sum_{i=1}^{n-k-1} a_{ji} OB_{i}, \,\, j=1,\dots, N,
#'  }
#' where \eqn{n} is the number of the knots (including the endpoints), \eqn{k} is the splines smoothness order, 
#' \eqn{N} is the number of the projected functions and  \eqn{OB_i} is the considered basis. 
#' The coefficient for the 'splinet' basis are always evaluated and thus, for example, 
#' `PFD=project(FD,knots)`
#' `ProjDataSplines=lincomb(PFD$coeff,PFD$basis)`
#' creates a \code{Splinets}-object made of the projections of the input functional data in 'FD'.
#' If the input parameter \code{basis} is given than the function utilizes this basis and does not 
...
The ", drop, " values at the arguments outside the projection range will not affect the projection.\n")
      ...
      if(is.null(proj$basis)){#the basis needs to be built
        ...
        if(type =='bs'){# the case of the B-splines (non-orthogonal basis)
          ...
          obs=splinet(Bsplines = bsp$bs,norm = T) #computing the orthogonal splines and the input B-splines are normalized
          #the first argument is the  B-spline basis unnormalized         
          
          #Computing the coefficients of the projection
          N=diag(1/sqrt(gramian(bsp$bs,norm_only=T))) #The normalizing matrix, It will be needed to express the coefficient 
          ...
          #   
          #   A=t(new_fdsp[,-1,drop=F])%*%DA #This is N x n_basis, it represents the inner products of 
        ...
        if(type=='bs'){
          ...
          #At the moment the basis is not normalized because the default 
          #in 'splinet' generates unnormalized version of the B-splines
          obs=splinet(Bsplines = proj$basis,norm = T) #computing the orthogonal splines and the input B-splines is normalized
          
          #Computing the coefficients of the projection
          N=diag(1/sqrt(gramian(proj$basis,norm_only=T))) #The normalizing matrix, can be the identity if th
      ...
      if(is.null(proj$basis)){#the basis needs to be built
        ...
        if(type =='bs'){# the case of the B-splines (non-orthogonal basis)
          ...
          #Computing the coefficients of the projection
          N=diag(1/sqrt(gramian(bsp$bs,norm_only=T))) #The normalizing matrix will be needed to express the result in the n
       ...
       #the end of the case when the basis is built 
          ...
          if(type=='bs'){
            ...
            obs=splinet(Bsplines = proj$basis,norm = T) #The output will have B-splines orthogonalized
            
            #Computing the coefficients of the projection
            N=diag(1/sqrt(gramian(proj$basis,norm_only=T))) #The normalizing matrix (could be identity if the original matrix
